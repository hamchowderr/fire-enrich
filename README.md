...
| `EVIDENCE_CHECK` | off | `1` (or `true`, `on`, `yes`, `enabled`) turns on a second evidence check: an evaluation model (`typesafe-ai/jev` on the AI Gateway) scores whether each finding's quote supports its value. `0`, `false`, `off`, `no` or `disabled` leave it off. When on, each finding with a value is one paid gateway call per run, and a failing or slow call can add up to 3 s per research group. It stays off by default until the question is checked on held-out findings. The AI SDK evaluation API it uses is experimental. |
